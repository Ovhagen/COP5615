defmodule BlockchainTest do
  ...
  test "Mine new block", data do
    # Mine a valid block
    {:ok, block} = Miner.build_mine_block(data.bc, data.miner_pkh, "test")
    :ok = Blockchain.verify_block(data.bc, block)
    
    # Serialize and deserialize the mined block
    assert block == Block.deserialize(Block.serialize(block))
    ...
    {:ok, block2} = Miner.build_mine_block(Map.update!(data.bc, :tip, &Map.put(&1, :hash, <<0::256>>)), data.miner_pkh, "test")
    ...
    {:ok, block} = Miner.build_mine_block(bc2, data.miner_pkh, "test")
  ...
  test "Add valid blocks to blockchain", data do
    ...
    {:ok, bc} = Blockchain.add_to_mempool(data.bc, data.tx)
    {:ok, block} = Miner.build_mine_block(bc, data.miner_pkh, "test")
    ...
    {:ok, block} = Miner.build_mine_block(data.bc, data.miner_pkh, "test")
    {:ok, bc} = Blockchain.add_block(data.bc, block)
    {:ok, block} = Miner.build_mine_block(bc, data.miner_pkh, "test2")
    {:ok, bc} = Blockchain.add_block(bc, block)
    {:ok, block} = Miner.build_mine_block(bc, data.miner_pkh, "test3")
    {:ok, bc} = Blockchain.add_block(bc, block)
    {:ok, block} = Miner.build_mine_block(bc, data.miner_pkh, "test4")
    {:ok, bc} = Blockchain.add_block(bc, block)
