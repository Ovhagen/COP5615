defmodule Blockchain.UTXO do
  ...
  defp split_spent(utxo, txs) do
    ...
    {stxo, utxo} = Map.split(utxo, txvins_to_keys(txs))
  ...
  def txvins_to_keys(txs), do: Enum.flat_map(txs, fn tx -> vins_to_keys(tx.vin) end)
  def vins_to_keys(vins), do: Enum.map(vins, &key/1)
  def txvouts_to_keys(txs) when is_list(txs), do: Enum.flat_map(txs, fn tx -> txvouts_to_keys(tx) end)
  def txvouts_to_keys(tx) do
    txid = Transaction.hash(tx)
    (for n <- 0..length(tx.vout)-1, do: {txid, n})
  end
