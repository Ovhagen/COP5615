defmodule BitcoinSimulatorWeb.ChartChannel.Monitor do
  require Logger
  use Agent
  @max_size 120
  ...
  defp chart_update(state, new_data) do
    case state[:msg] do
      nil   ->
        new_data |> Enum.map(fn {key, _val} -> {key, [[Enum.at(new_data[key], 0)], [Enum.at(new_data[key], 1)]]} end) |> Enum.into(%{})
      _data ->
        ...
        new_state = (if current_length >= @max_size do
          state
          |> Enum.map(fn {key, [[_|x],[_|y]]} -> {key, [x,y]} end) |> Enum.into(%{})
        else
          state
        end)
