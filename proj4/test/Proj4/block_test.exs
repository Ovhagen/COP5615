defmodule Proj4.BlockTest do
  ...
  test "Successfully verifying a block", %{tx_test: tx_test, prev_hash: prev_hash, nonce: nonce, size_test: size_test} do
    tree = MerkleTree.makeMerkle(tx_test)
    block = Block.createBlock(tx_test, tree.root.hash_value, prev_hash, nonce)
    block_hash = Block.generate_block_hash(block.block_header) |> IO.inspect
    assert(Block.verifyBlock(block, block_hash))
  end

  # test "Fail block verification on difficulty", %{tx_test: tx_test, prev_hash: prev_hash, nonce: nonce, size_test: size_test} do
  #   tree = MerkleTree.makeMerkle(tx_test)
  #   block = Block.createBlock(tx_test, tree.root.hash_value, prev_hash, nonce)
  #   block_hash = Block.generate_block_hash(block.block_header)
  #   assert_raise(Block.DiffError, fn () -> Block.verifyBlock(block, block_hash) end)
  # end

  test "Fail block verification on block hash", %{tx_test: tx_test, prev_hash: prev_hash, nonce: nonce, size_test: size_test} do
    tree = MerkleTree.makeMerkle(tx_test)
    block = Block.createBlock(tx_test, tree.root.hash_value, prev_hash, nonce)
    block_hash = Block.generate_block_hash(block.block_header) <> "fail"
    assert_raise(Block.BlockHashError, fn () -> Block.verifyBlock(block, block_hash) end)
  end

  test "Fail block verification on merkle root hash", %{tx_test: tx_test, prev_hash: prev_hash, nonce: nonce, size_test: size_test} do
    tree = MerkleTree.makeMerkle(tx_test)
    block = Block.createBlock(tx_test, tree.root.hash_value <> "fail", prev_hash, nonce)
    block_hash = Block.generate_block_hash(block.block_header)
    assert_raise(Block.MerkleRootError, fn () -> Block.verifyBlock(block, block_hash, :full) end)
  end
