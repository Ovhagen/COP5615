defmodule Blockchain do
  ...
  defp remove_from_mempool(bc, txs, txids) do
    {stxo, utxo} = Map.split(bc.utxo, Blockchain.UTXO.txvins_to_keys(txs))
  ...
  @spec get_block_by_hash(t | Blockchain.Link.t | nil, Crypto.hash256) :: {:ok, Block.t} | :error
  def get_block_by_hash(%Blockchain{} = bc, hash), do: get_block_by_hash(bc.tip, hash)
  def get_block_by_hash(nil, _hash), do: :error
  def get_block_by_hash(link, hash), do: if(link.hash == hash, do: {:ok, link.block}, else: get_block_by_hash(link.prev, hash))
  
  @doc """
  Returns a list of block hashes in increasing height starting from the genesis block.
  """
  @spec hash_list(t) :: [Crypto.hash256, ...]
  def hash_list(bc), do: hash_list(bc.tip, [])
  defp hash_list(nil, hashes), do: hashes
  defp hash_list(link, hashes), do: hash_list(link.prev, [link.hash] ++ hashes)
